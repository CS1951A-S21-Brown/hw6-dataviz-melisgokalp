...
let svg3 = d3.select("#graph3")
    ...
    .attr("height",graph_3_height+ margin.top*4)
    .append("g")
    .attr("transform", `translate(${(margin.left/2+5) },${margin.top+10})`);

let title_3 = svg3.append("text")
    .attr("transform", `translate(${(graph_3_width/3 )}, ${-20})`)  
...
d3.csv(filename).then(function(data) {
    ...
    var x = d3.scaleLinear()
        ...
        .range([ 0,  graph_3_width-margin.left/2]);
    svg3.append("g")
    ...
    let publishers = svg3.append("g").selectAll("text").data(data);

    // add top publisher names on bars
    publishers.enter()
        .append("text")
        .merge(publishers)
        .attr("x", function(d) { return x(0) + 10; })
        .attr("y", function(d) { return y(d.genre)+20; })
